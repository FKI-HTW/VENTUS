...
namespace Plugins.QuickOutline.Scripts
{
    ...
    {
        private static readonly Dictionary<Mesh, int> registeredMeshes = new();
        ...
        public bool UpdateOnMeshChanged
        {
            get => _updateOnMeshChanged;
            set => _updateOnMeshChanged = value;
        }
        ...
        [SerializeField] private bool _updateOnMeshChanged;
        ...
        private void Update()
        {
            if (_updateOnMeshChanged)
            {
                // Retrieve or generate smooth normals
                if (LoadSmoothNormals())
                {
                    // Apply material properties immediately
                    needsUpdate = true;
                }
        ...
        private bool LoadSmoothNormals()
        {
            ...
            foreach (var meshFilter in GetComponentsInChildren<MeshFilter>())
            {
                // Skip if mesh doesn't have vertices
                if (meshFilter.sharedMesh == null || meshFilter.sharedMesh.vertexCount == 0) continue;
                
                // Skip if smooth normals have already been adopted to the same vertex count
                if (registeredMeshes.TryGetValue(meshFilter.sharedMesh, out int value) && value == meshFilter.sharedMesh.vertexCount) continue;
                registeredMeshes.Add(meshFilter.sharedMesh, meshFilter.sharedMesh.vertexCount);
                changedMesh = true;
            ...
            {
                // Skip if mesh doesn't have vertices
                if (skinnedMeshRenderer.sharedMesh == null || skinnedMeshRenderer.sharedMesh.vertexCount == 0) continue;
                
                // Skip if smooth normals have already been adopted to the same vertex count
                if (registeredMeshes.TryGetValue(skinnedMeshRenderer.sharedMesh, out int value) && value == skinnedMeshRenderer.sharedMesh.vertexCount) continue;
                registeredMeshes.Add(skinnedMeshRenderer.sharedMesh, skinnedMeshRenderer.sharedMesh.vertexCount);
                changedMesh = true;
